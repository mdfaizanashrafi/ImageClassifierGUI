...
pip install tensorflow matplotlib scikit-learn opencv-python pillow reportlab pyinstaller
...
class Config:
    ...
    CHECKPOINT_PATH="checkpoints/best_models.h5"
    FINAL_MODEL_PATH="models/final_models.h5"
...
def build_simple_cnn(input_size=(32,32,3)):
  model = tf.keras.Sequential([
      ...
      tf.keras.layers.Conv2(32,(3,3), activation='relu',padding='same',),
      ...
      tf.keras.layers.Conv2D(128,(3,4),activation='relu',padding='same'),
      tf.keras.layers.BatchNormalization(),
      tf.keras.layers.Conv2D(128,(3,4),activation='relu',padding='same'),
...
def plot_training_history(history):
  ...
  plt.title('Trainingn and Validation Loss')
  plt.legend()
  plt.tight_layout()
...
class ImageClassifierApp(tk.Tk):
  ...
  def __init__(self):
    super().__init__()
    self.title("AI Image Classifier with Webcam + Custom Prediction")
  ...
  def create_widgets(self):
    ...
    ttk.Button(control_frame,
               text="Evaluate Model",
               command= self.evaluate_model).grid(row=0,column=3,padx=5)
    ...
    self.pred_label= ttk.Label(pred_frame,
                               text="Prediction: N/A",
                               font=("Arial", 14))
    ...
    ttk.Button(pred_frame,
               ...
               command= self.open_webcam).pack(pady=5)
    ...
    ttk.Button(button_frame,
               ...
               command= self.plot_training).pack(side=tk.LEFT, padx=5)
    ttk.Button(button_frame,
               text="Plot Confusion Matrix",
               command= self.plot_confusion).pack(side=tk.LEFT,padx=5)
    ttk.Button(button_frame,
               text="Export Report as PDF",
               command= export_plot_as_pdf).pack(side=tk.LEFT,padx=5)
    ttk.Button(button_frame,
               text="Save Model", command= self.save_model).pack(side=tk.LEFT, padx=5)
    ttk.Button(button_frame,
               text="Load Model",
    ...
    self.log_text= scrolledtext.ScrolledText(self, height=10, weight=110)
  ...
  def evaluate_model(self):
    if self.model is None:
      self.log_text.insert(tk.END,
                           "[ERROR] No model loaded or trained yet. \n")
      return

    loss, accuracy= self.model.evaluate(self.x_test, self.y_test, verbose=0)
    self.log_text.insert(tk.END,
                         f"Test Accuracy: {accuracy: .4f}, Test Loss: {loss:.4f}\n")
